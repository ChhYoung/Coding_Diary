...
- `LoopNode`和`BranchNode`  :  控制节点实现控制流, 
  ...
  - BranchNode : 相当于一个多对一的开关，在BranchNode的父节点中选一个作为子节点的依赖，从而实现分支控制,  其中在前馈图中`BranchNode`为其子节点的前驱,  这样每次由虚拟图构造计算图时在`forward`中完成分支选择从而进行计算图构建
...
**RNN**:  结构如下, 用此结构实现一个10进制加法结果转成二进制的网络
