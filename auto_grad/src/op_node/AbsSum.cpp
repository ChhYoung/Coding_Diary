...
AbsSum::AbsSum(std::string type, std::string id, std::string idx) :OperatorNode(type, id, idx) {
}

void AbsSum::op() {
	Tensor* parent_output = ((OperatorNode*)m_parents[0])->m_output;
	std::vector<int> shape(2); shape[0] = 1; shape[1] = 1;
...
void AbsSum::grad_op() {
	Tensor* parent_output = ((OperatorNode*)m_parents[0])->m_output;
	std::vector<int> shape(2); shape[0] = 1; shape[1] = parent_output->m_size;
